...
% %随机选择
positive_indices=randperm(50,35);
negative_indices=randperm(50,35)+50;
remaining_indices=setdiff(1:100,[positive_indices,negative_indices]);
train_X=tensor_data([positive_indices,negative_indices],:);
test_X=tensor_data(remaining_indices,:);
train_labels=label([positive_indices,negative_indices],:);
test_labels=[-ones(15,1);ones(15,1)];
...
%         sigmarange =[-8:1:8];
        sigmarange =6;
        Crange = [-8:1:8];alpha_int=zeros(N,1);
        for sigma_i = 1:size(sigmarange,2)
            sigma=2^sigmarange(sigma_i);
            tic
            [ K] = kernel_mat( X, N,d,sigma,weight,flag);%学到的核函数
            fprintf('kernel matrix costs time is   ')
            toc
            for C_i = 1:size(Crange,2)
                sigma=2^sigmarange(sigma_i);
                C=2^Crange(C_i);
%                 [ alpha, b] = svm_solver( K, Y, C, N);%改成DCDMG算法
%                 H= (Y * Y') .* K;
%                 f = -ones(length(Y),1);tol_num=1000;
                if C_i>1
                    alpha_int=2^Crange(C_i)/2^Crange((C_i)-1)*model.x;
                end
%                 [alpha, model] = DCDMG(H, N, C, tol_num, f, alpha_int);
                [alpha, model] = DCDMG(Y,C,alpha_int,K);
                time(C_i)=model.time
...
%                 Ypred = predict(validdata, alpha, b, X, Y, sigma,d,weight,flag);
                Ypred = predict(validdata, alpha,  X, Y, sigma,d,weight,flag);
...
%                 Ypred = predict(XX, alpha, b, X, Y, sigma,d,weight,flag);
                Ypred = predict(XX, alpha,  X, Y, sigma,d,weight,flag);
                Ypred(Ypred>0)=1;
                Ypred(Ypred<0)=-1;
                diff=Ypred-test_labels;
                diff(diff~=0)=1;
                test_error(sigma_i,C_i)=sum(diff)/N_test;
                Accuracy(sigma_i,C_i)=1-test_error(sigma_i,C_i)
            end
        end
    end
end
