...
%     if l<=5
        ...
        TT_CP_data{i,1}{2,1} = reshape(permute(data_TT{i}{2},[2,1,3]),dimn(2),l*l);%重新排列第二个核心张量
        TT_CP_data{i,1}{3,1} = kron(reshape(permute(data_TT{i}{3},[2,1]),dimn(3),l),ones(1,l));%
%     else 
%         TT_CP_data{i,1}{1,1} = kron(ones(1,l),reshape(permute(data_TT{i}{1},[2,1,3]),dimn(1),5));%
%         TT_CP_data{i,1}{2,1} = reshape(permute(data_TT{i}{2},[2,1,3]),dimn(2),l*5);%重新排列第二个核心张量
%         TT_CP_data{i,1}{3,1} = kron(reshape(permute(data_TT{i}{3},[2,1]),dimn(3),l),ones(1,5));%
%     end
    normA{i,1}=sqrt(sum(TT_CP_data{i,1}{1,1}.*conj(TT_CP_data{i,1}{1,1})));  % Frobenius norm of each column of A
    normB{i,1}=sqrt(sum(TT_CP_data{i,1}{2,1}.*conj(TT_CP_data{i,1}{2,1})));
    normC{i,1}=sqrt(sum(TT_CP_data{i,1}{3,1}.*conj(TT_CP_data{i,1}{3,1})));
    prod_norm{i,1}=normA{i,1}.*normB{i,1}.*normC{i,1};
    Scale_mat{i,1}=diag(prod_norm{i,1}.^(1/3)); %计算所有秩一张量的范数
