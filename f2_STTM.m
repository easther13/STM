...
function [e, labels,W,b]=f2_STTM(train_X,train_labels,W_r,C);
...
X=cat(4, X{:});
%%Salines
X=reshape(permute(X, [2 3 4 1]),5,5,224,60);%61*73*61*N tensor
...
d=length(W_r)-1;
for i=1:length(X) %get the feature dimension, namely 61*73*61
    fd(i)=size(X{i},2);
end
for i=1:length(X)-1 
    Xd(i)=size(X{i},3);
...
%             'ClassNames',{'0','1'},'OutlierFraction',0.02);%,'OptimizeHyperParameters','auto'?%'solver','L1QP'?You cannot use QP for robust learning.
        f = -ones(length(train_labels),1);
%         C=para.Cp(j);
        w_pre = zeros(n+1,1);
        object_pre = 0;X_C = zeros(1,n+1);Y_C =0;
        alpha_int = zeros(m,1);tol_num=1000;
        model = DCDM(x,train_labels,X_C,Y_C,C,f,w_pre,object_pre,tol_num, alpha_int);
        w=model.w(1:end-1,1); 
        SVMModel.Bias=model.w(end,1);
